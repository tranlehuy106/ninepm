...
from ninepm.app.ninepm_mobile_api.dtos.auth_conf import AuthTokenStorageConfig
...
class RedisClientGenerator(AbstractRedisClientGenerator):
    ...
    @staticmethod
    def __get_connection_config(config: dict) -> dict:
        now = timezone.now()
        ...
        if key not in RedisClientGenerator._pools:
            ...
            RedisClientGenerator._pools[key] = _PoolItem(
